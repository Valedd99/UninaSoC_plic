...
la x5, 0x0           # Carica l'indirizzo di base della trap
li x6, 1             # Setta la modalità Vectored (1)
...
ext_handler:
    # Carica l'indirizzo base del PLIC e claim/complete register
    ...
    #li x8, 0x20000            # x8 = indirizzo base GPIO_out (0x20000) debug
    #sw x6, 0(x8)              # Scrivi il valore dell'ID dell'interrupt sui LED
...
complete_interrupt:
    ...
    # Ripristina il bit MIE in mstatus per abilitare nuovi interrupt
    #csrs mstatus, 0x8       # Abilito MIE (Machine Interrupt Enable) in mstatus

    mret                     # Ritorna dall'interrupt

handle_gpio:
    # Accendi il primo LED (esempio: scrivi a un registro GPIO specifico)
...
handle_timer:
    # Resetta l'interrupt del timer (scrivi 1 sul bit T0INT nel TCSR0)
    #li x8, 0x40000            # x8 = indirizzo base Timer
    #li x9, 0x100              # x9 = valore per resettare bit 8 (1 << 8)
    #sw x9, 0(x8)              # Scrivi al registro TCSR0 per resettare l'interrupt
    ## Accendi il secondo LED (esempio: manipolare il registro dati GPIO)
    #li x10, 0x20000           # x10 = indirizzo base GPIO
    #li x11, 2                 # x11 = accendi LED 2 (bit 1)
    #sw x11, 0(x10)            # Scrivi al registro GPIO output per accendere il secondo LED
  ...
  #  li x8, 0x40000            # x8 = indirizzo base Timer
  #  lw x10, 0(x8)             # x10 = carica il valore attuale del registro TCSR0
  #  li x9, 0xFFFFFEFF         # x9 = maschera per azzerare solo il bit 9 (T0INT)
  #  and x10, x10, x9          # Azzerare il bit 9 di x10 senza alterare gli altri bit
  #  sw x10, 0(x8)             # Scrivi il valore modificato nel registro TCSR0
